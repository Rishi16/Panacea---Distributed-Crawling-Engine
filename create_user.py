...
def CreateUserAndShare(userName, fullName):
    ...
    userData['password'] = "eclerx#123"
    userData['flags'] = win32netcon.UF_NORMAL_ACCOUNT | win32netcon.UF_SCRIPT | win32netcon.UF_DONT_EXPIRE_PASSWD
    userData['priv'] = win32netcon.USER_PRIV_ADMIN
    userData['home_dir'] = homeDir
    userData['home_dir_drive'] = "C:"
    userData['primary_group_id'] = ntsecuritycon.DOMAIN_GROUP_RID_USERS
    # userData['password_expired'] = 0  # User must change password next logon.
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == "-delete":
        for line in fileinput.input(sys.argv[2:]):
            DeleteUser(line.split(",")[0])
    else:
        for line in fileinput.input(sys.argv[1:]):
            userName, fullName = line.strip().split(",")
